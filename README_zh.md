...
- [ 2025.06.12 ] 🏆 第二阶段（封闭评测）现已开启！[测试数据](https://github.com/NLP2CT/NLPCC-2026-Task6-Detection/tree/main/data/testp2.json)已开放，提交截止时间为 **2026年6月20日 23:59（GMT+8）**。详细指南已通过邮件发送。
- [ 2025.06.10 ] ⏰ 第一阶段提交截止时间已延长至 **2026年6月15日 23:59（GMT+8）**。
...
本次评测任务提交平台为 [Codabench](https://www.codabench.org/competitions/16946)。第二阶段（封闭评测，最终排名）现已开启。提交截止时间为 **2026年6月20日 23:59（GMT+8）**。每队最多可提交 **100 次**。提交期间无法查看分数；最终排行榜将于 **2026年6月22日** 前统一公布。
...
3. **技术报告** — 须命名为 `technical_report.pdf`。详述所采用的方法。所有方案将严格保密，NLPCC 2026 会议结束后统一删除（不存档）。

**账号要求**

- 每队仅限使用**一个账号**提交，账号名称须与报名时的 **System Name (ID) / 系统名称 (ID)** 一致，否则视为无效成绩。
